  ...
  const execute = async () => {
    ...
    try {
      if (activeTool === "image-pdf") {
        ...
        for (const item of files.filter((entry) => entry.file.type.startsWith("image/"))) {
        ...
        for (const item of files.filter((entry) => entry.file.type === "application/pdf")) { const source = await PDFDocument.load(await readFile(item.file)); const pages = await merged.copyPages(source, source.getPageIndices()); pages.forEach((page) => merged.addPage(page)); }
    ...
    } catch { toast.error("Fileটি process করা যায়নি। / We could not process this file."); }
